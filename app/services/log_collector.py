...
class LogCollector:
    """
    Klasa odpowiedzialna za pobieranie logów z zdalnych maszyn (SSH) oraz lokalnego Windowsa.
    Implementuje logikę normalizacji danych do wspólnego formatu słownika.
    """
    ...
    def get_linux_logs(ssh_client, last_fetch_time=None):
        """
        Pobiera logi z demona SSH na Linuxie przy użyciu `journalctl`.
        Wykorzystuje flagę output json dla łatwiejszego parsowania.
        """
        logs = []
        ...
        if last_fetch_time:
            since_timestamp = int(last_fetch_time.timestamp())
    ...
    def get_windows_logs(win_client, last_fetch_time=None):
        ...
        # Filtrowanie po czasie (Python side) - naprawia problem duplikatów
        if last_fetch_time:
            # last_fetch_time z bazy jest UTC (aware).
            # Logi z Windowsa są w czasie lokalnym (naive).
            # Musimy przekonwertować last_fetch na czas lokalny SYSTEMU, żeby porównanie miało sens.
            local_cutoff = last_fetch_time.astimezone().replace(tzinfo=None)
            
            logs = [log for log in logs if log['timestamp'] > local_cutoff]
