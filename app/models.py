...
class Host(db.Model):
    """
    Reprezentuje monitorowane urządzenie (Linux/Windows).
    """
    __tablename__ = 'hosts'
    id = db.Column(db.Integer, primary_key=True)
...
class LogSource(db.Model):
    """
    Przechowuje stan pobierania logów dla danego hosta (np. kiedy ostatnio pobrano logi).
...
class LogArchive(db.Model):
    """
    Rejestr plików Parquet zapisanych w systemie plików.
    Umożliwia śledzenie historii pobrań (Forensics).
    """
...
class IPRegistry(db.Model):
    """
    Baza Threat Intelligence (TI). Pamięta reputację adresów IP (Zaufany/Zbanowany).
    """
