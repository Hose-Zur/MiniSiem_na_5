...
export async function initDashboard() {
    ...
    if (archiveBtn) {
        archiveBtn.addEventListener('click', handleArchiveAlerts);
    }
}

async function handleArchiveAlerts() {
    window.location.href = '/api/alerts/archive';
}

async function initChart() {
    const ctx = document.getElementById('topIpsChart');
    if (!ctx) return; // Jeśli nie ma elementu na stronie, przerwij

    try {
        const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');

        const response = await fetch('/api/stats/top-ips', {
            headers: {
        ...
        if (!chartData.labels || chartData.labels.length === 0) {
            ctx.parentNode.innerHTML = '<div class="text-center text-body-secondary py-4">Brak danych o atakach do wyświetlenia.</div>';
            return;
        }
        ...
        console.error("Wykres error:", error);
        ctx.parentNode.innerHTML = '<div class="alert alert-warning small">Nie udało się załadować wykresu.</div>';
    }
}
...
async function refreshHostsList() {
    ...
    try {
        ...
        if (hosts.length === 0) {
            createEl('div', ['p-4', 'text-center', 'text-body-secondary'], 'Brak hostów. Skonfiguruj system w panelu admina.', hostsContainer);
            return;
        }
...
function renderDashboardRow(host) {
    ...
    createEl('small', ['text-body-secondary', 'text-truncate'], host.ip_address, details);

    // KOLUMNA 2: STATUS
    const colStatus = createEl('div', ['col-5', 'px-2'], '', row);
    createEl('div', ['text-body-secondary', 'small', 'text-center', 'fst-italic'], 'Kliknij Status...', colStatus);
    ...
    const logsBtn = createEl('button', ['btn', 'btn-outline-secondary'], 'Logi', btnGroup);
...
async function handleCheckStatusFancy(host, container, btn) {
    ...
    createEl('div', ['text-body-secondary', 'small', 'text-center'], 'Łączenie...', container);
...
async function handleFetchLogs(host, btn) {
    ...
    try {
        ...
        if (result.alerts_found > 0) {
            btn.innerHTML = '⚠️ ' + result.alerts_found;
...
function addBadge(parent, label, value, colorClass) {
    const box = createEl('div', ['text-center', 'border', 'rounded', 'bg-body-tertiary', 'py-1'], '', parent);
    box.style.width = '24%';
    const lbl = createEl('div', ['text-body-secondary', 'text-uppercase'], label, box);
...
async function refreshAlertsTable() {
    ...
    try {
        ...
        if (alerts.length === 0) {
            ...
            const cell = createEl('td', ['text-center', 'text-body-secondary', 'py-3'], 'Brak alertów.', row);
            cell.colSpan = 6;
            return;
        }
        ...
        const row = createEl('tr', [], '', alertsBody);
        const cell = createEl('td', ['text-center', 'text-danger', 'py-3'], 'Błąd ładowania alertów. Sprawdź konsolę.', row);
        cell.colSpan = 6;
    }
}