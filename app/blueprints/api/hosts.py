import time
from flask import Blueprint, jsonify, request, current_app, Response
import json
from datetime import timezone, datetime
import os
from sqlalchemy import func

from flask_login import login_required

from app.models import Host, LogSource, LogArchive, Alert, IPRegistry
from app.services.remote_client import RemoteClient
from app.services.win_client import WinClient
from app.services.log_collector import LogCollector
from app.services.data_manager import DataManager
from app.services.log_analyzer import LogAnalyzer
from app.extensions import db

api_bp = Blueprint("api_hosts", __name__)

# --- CRUD HOSTS (GOTOWE - ABY UI DZIAŁAŁO) ---

@api_bp.route("/hosts", methods=["GET"])
@login_required
def get_hosts():
    hosts = Host.query.all()
    return jsonify([h.to_dict() for h in hosts])

@api_bp.route("/hosts", methods=["POST"])
@login_required
def add_host():
    data = request.get_json()
    if not data: return jsonify({"error": "Brak danych"}), 400
    if Host.query.filter_by(ip_address=data.get("ip_address")).first():
        return jsonify({"error": "IP musi być unikalne"}), 409
    new_host = Host(hostname=data.get("hostname"), ip_address=data.get("ip_address"), os_type=data.get("os_type"))
    db.session.add(new_host)
    db.session.commit()
    return jsonify(new_host.to_dict()), 201

@api_bp.route("/hosts/<int:host_id>", methods=["DELETE"])
@login_required
def delete_host(host_id):
    host = Host.query.get_or_404(host_id)
    db.session.delete(host)
    db.session.commit()
    return jsonify({"message": "Usunięto hosta"}), 200

@api_bp.route("/hosts/<int:host_id>", methods=["PUT"])
@login_required
def update_host(host_id):
    host = Host.query.get_or_404(host_id)
    data = request.get_json()
    if 'hostname' in data: host.hostname = data['hostname']
    if 'ip_address' in data: host.ip_address = data['ip_address']
    if 'os_type' in data: host.os_type = data['os_type']
    db.session.commit()
    return jsonify(host.to_dict()), 200

# --- MONITORING LIVE (GOTOWE) ---

@api_bp.route("/hosts/<int:host_id>/ssh-info", methods=["GET"])
@login_required
def get_ssh_info(host_id):
    """
    Pobiera informacje o wykorzystaniu zasobów (RAM, CPU, Dysk) z hosta Linux przez SSH.
    """
    host = Host.query.get_or_404(host_id)
    ssh_user = current_app.config.get("SSH_DEFAULT_USER", "vagrant")
    ssh_port = current_app.config.get("SSH_DEFAULT_PORT", 2222)
    ssh_key = current_app.config.get("SSH_KEY_FILE")
    ssh_password = current_app.config.get("SSH_PASSWORD")
    try:
        with RemoteClient(host=host.ip_address, user=ssh_user, port=ssh_port, key_file=ssh_key, password=ssh_password) as remote:
            ram_out, _ = remote.run("free -m | grep Mem | awk '{print $7}'")
            disk_percentage, _ = remote.run("df -h | grep '/$' | awk '{print $5}'")
            if not disk_percentage: disk_percentage, _ = remote.run("df -h | grep '/dev/sda1' | awk '{print $5}'")
            disk_total, _ = remote.run("df -h | grep '/dev/sda1' | awk '{print $2}'")
            cpu_load, _ = remote.run("uptime | awk -F'load average:' '{ print $2 }' | cut -d',' -f1")
            uptime_seconds_str, _ = remote.run("cat /proc/uptime | awk '{print $1}'")
            uptime_formatted = "N/A"
            try:
                total_seconds = float(uptime_seconds_str)
                hours = int(total_seconds // 3600)
                minutes = int((total_seconds % 3600) // 60)
                uptime_formatted = f"{hours}h {minutes}m"
            except: pass

            return jsonify({
                "free_ram_mb": ram_out.strip(), "disk_info": disk_percentage.strip(),
                "disk_total": disk_total.strip(), "cpu_load": cpu_load.strip(), "uptime_hours": uptime_formatted
            }), 200
    except Exception as e:
        return jsonify({"error": f"Błąd połączenia: {str(e)}"}), 500

@api_bp.route("/hosts/<int:host_id>/windows-info", methods=["GET"])
@login_required
def get_windows_info(host_id):
    """
    Pobiera informacje o zasobach z lokalnego hosta Windows (gdzie uruchomiony jest serwer Flask).
    Używa biblioteki psutil.
    """
    import psutil
    host = Host.query.get_or_404(host_id)
    if host.os_type != "WINDOWS": return jsonify({"error": "Wrong OS"}), 400
    try:
        mem = psutil.virtual_memory()
        free_ram_mb = str(round(mem.available / (1024 * 1024)))
        cpu_load = f"{psutil.cpu_percent(interval=0.1)}%"
        try:
            usage = psutil.disk_usage("C:\\")
            disk_percentage = f"{usage.percent}%"
            disk_total = f"{round(usage.total / (1024**3), 1)}GB"
        except:
            disk_percentage, disk_total = "N/A", "?"
        boot_time = datetime.fromtimestamp(psutil.boot_time())
        uptime_seconds = (datetime.now() - boot_time).total_seconds()
        hours = int(uptime_seconds // 3600)
        minutes = int((uptime_seconds % 3600) // 60)
        return jsonify({
            "free_ram_mb": free_ram_mb, "disk_info": disk_percentage,
            "disk_total": disk_total, "cpu_load": cpu_load, "uptime_hours": f"{hours}h {minutes}m"
        }), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# ===================================================================
# MIEJSCE NA TWOJĄ IMPLEMENTACJĘ (ZADANIE 2 i 3)
# ===================================================================

@api_bp.route("/hosts/<int:host_id>/logs", methods=["POST"])
@login_required
def fetch_logs(host_id):
    """
    Pobiera logi z hosta (SSH/WMI) w trybie przyrostowym.
    Zapisuje je do pliku Parquet i uruchamia analizę.
    """
    host = Host.query.get_or_404(host_id)
    log_source = LogSource.query.filter_by(host_id=host.id, log_type='security').first()
    if not log_source:
        log_source = LogSource(host_id=host.id, log_type='security', last_fetch=None)
        db.session.add(log_source)
        db.session.commit()

    try:
        new_logs = []
        start_time = log_source.last_fetch.replace(tzinfo=timezone.utc) if log_source.last_fetch else None

        if host.os_type == 'LINUX':
            ssh_user = current_app.config.get("SSH_DEFAULT_USER", "vagrant")
            ssh_port = current_app.config.get("SSH_DEFAULT_PORT", 2222)
            ssh_key = current_app.config.get("SSH_KEY_FILE")
            ssh_password = current_app.config.get("SSH_PASSWORD")
            with RemoteClient(host=host.ip_address, user=ssh_user, port=ssh_port, key_file=ssh_key, password=ssh_password) as remote:
                new_logs = LogCollector.get_linux_logs(remote, last_fetch_time=start_time)

        elif host.os_type == 'WINDOWS':
            # client = WinClient(host.ip_address) # Zakładamy, że WinClient nie wymaga parametrów
            client = WinClient()
            new_logs = LogCollector.get_windows_logs(client, last_fetch_time=start_time)

        if not new_logs:
            return jsonify({"message": "Brak nowych logów do pobrania.", "logs_found": 0, "alerts_found": 0}), 200

        utc_now = datetime.now(timezone.utc)
        file_name, logs_count = DataManager.save_logs_to_parquet(new_logs, host.id)

        if file_name:
            new_archive = LogArchive(host_id=host.id, filename=file_name, timestamp=utc_now)
            db.session.add(new_archive)

            log_source.last_fetch = utc_now
            db.session.commit()

            alerts_count = LogAnalyzer.analyze_parquet(file_name, host.id)

            return jsonify({
                "message": f"Pobrano {logs_count} logów. Stworzono {alerts_count} alertów.",
                "logs_found": logs_count,
                "alerts_found": alerts_count
            }), 200
        else:
            return jsonify({"message": "Nie udało się zapisać logów.", "logs_found": 0, "alerts_found": 0}), 500

    except Exception as e:
        current_app.logger.error(f"Błąd podczas pobierania logów dla hosta {host_id}: {e}")
        return jsonify({"error": str(e)}), 500


# --- THREAT INTEL & DASHBOARD API ---

@api_bp.route("/ips", methods=["GET"])
@login_required
def get_ips():
    """Zwraca listę wszystkich adresów IP z rejestru."""
    ips = IPRegistry.query.order_by(IPRegistry.last_seen.desc()).all()
    return jsonify([ip.to_dict() for ip in ips])

@api_bp.route("/ips", methods=["POST"])
@login_required
def add_ip():
    """Dodaje nowy adres IP do rejestru."""
    data = request.get_json()
    if not data or 'ip_address' not in data or 'status' not in data:
        return jsonify({"error": "Brak wymaganych danych: ip_address, status"}), 400

    if IPRegistry.query.filter_by(ip_address=data['ip_address']).first():
        return jsonify({"error": "Adres IP już istnieje w rejestrze"}), 409

    new_ip = IPRegistry(
        ip_address=data['ip_address'],
        status=data['status'],
        last_seen=datetime.now(timezone.utc)
    )
    db.session.add(new_ip)
    db.session.commit()
    return jsonify(new_ip.to_dict()), 201

@api_bp.route("/ips/<int:ip_id>", methods=["PUT"])
@login_required
def update_ip(ip_id):
    """Aktualizuje status istniejącego adresu IP."""
    ip_entry = IPRegistry.query.get_or_404(ip_id)
    data = request.get_json()
    if not data or 'status' not in data:
        return jsonify({"error": "Brak wymaganych danych: status"}), 400

    ip_entry.status = data['status']
    ip_entry.last_seen = datetime.now(timezone.utc)
    db.session.commit()
    return jsonify(ip_entry.to_dict())

@api_bp.route("/ips/<int:ip_id>", methods=["DELETE"])
@login_required
def delete_ip(ip_id):
    """Usuwa adres IP z rejestru."""
    ip_entry = IPRegistry.query.get_or_404(ip_id)
    db.session.delete(ip_entry)
    db.session.commit()
    return jsonify({"message": "Usunięto adres IP"}), 200

@api_bp.route("/alerts", methods=["GET"])
@login_required
def get_recent_alerts():
    """Zwraca 20 ostatnich alertów, dołączając nazwę hosta."""
    alerts = db.session.query(Alert, Host.hostname).join(Host, Alert.host_id == Host.id).order_by(Alert.timestamp.desc()).limit(20).all()

    results = []
    for alert, hostname in alerts:
        alert_dict = alert.to_dict()
        alert_dict['hostname'] = hostname
        results.append(alert_dict)

    return jsonify(results)

@api_bp.route("/alerts/archive", methods=["GET"])
@login_required
def archive_alerts():
    """
    Eksportuje wszystkie alerty do pliku JSON.
    Wymaganie na ocenę 5.0 (Punkt 2).
    """
    alerts = db.session.query(Alert, Host.hostname).join(Host, Alert.host_id == Host.id).order_by(Alert.timestamp.desc(), Alert.id.desc()).all()
    
    results = []
    for alert, hostname in alerts:
        alert_dict = alert.to_dict()
        alert_dict['hostname'] = hostname
        results.append(alert_dict)

    json_str = json.dumps(results, indent=4, ensure_ascii=False)

    return Response(
        json_str,
        mimetype="application/json",
        headers={"Content-Disposition": "attachment;filename=alerts_archive.json"}
    )

@api_bp.route("/stats/top-ips", methods=["GET"])
@login_required
def get_top_ips():
    try:
        results = db.session.query(
            Alert.source_ip,
            func.count(Alert.id).label('count')
        ).group_by(Alert.source_ip) \
            .order_by(func.count(Alert.id).desc()) \
            .limit(5).all()

        data = {
            'labels': [row.source_ip for row in results if row.source_ip],
            'data': [row.count for row in results if row.source_ip]
        }

        return jsonify(data)
    except Exception as e:
        return jsonify({'error': str(e)}), 500